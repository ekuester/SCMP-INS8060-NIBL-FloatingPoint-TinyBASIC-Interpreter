; compare NIBL assembler listing by Roger Marin, January 2008
; analogously rewritten by Erich Küster, Krefeld/Germany
;
;   .TITLE  NIBL,'12/17/76'
;   .LIST   1
;
...
VARS      =  0x2000 +28             ;NIBL VARIABLES A-Z
...
CHEAT:  LD      @1(P3)
        ...
        LDE                   ;THE I.L. PC BY 2
        ANI     0x1F          ;REMOVE FLAG FROM INSTRUCTION
        ORI     $/256         ;TURN INTO ACTUAL ADDRESS,
        XPAH    P3            ;PUT BACK INTO P3
        ST      PCHIGH(P2)
        LDE
        ANI     0xE0          ;CHECK IF I.L. INSTRUCTION
...
LOOP4:  LD      @1(P1)        ;NO - SCAN THROUGH TO
        JP      LOOP4         ;NEXT MESSAGE
        JMP     NMSG
MSG0:   LD      @1(P1)        ;GET MESSAGE CHAR
        XPPC    P3            ;PRINT IT
        LD      -1(P1)        ;IS MESSAGE DONE?
        JP      MSG0          ;NO - GET NEXT CHAR
...
SKIP0:  LD      RUNMOD(P2)    ;DON'T PRINT LINE #
...
MM4:    LD      TEMP(P2)      ;CHECK SIGN WORD
        ...
        CAD     1(P3)
        ST      1(P3)
EXIT0:  LD      0(P3)         ;PUT PRODUCT ON TOP
        ST      -4(P3)        ;OF STACK
        LD      1(P3)
        ST      -3(P3)
...
TSTVAR: LD      @1(P1)
        ...
        JP      TVMAY         ;IF POS, MAY BE VARIABLE
...
TVMAY:  XAE                   ;SAVE VALUE (0-25)
        ...
        CAI     'A'-'Z'-1
        JP      TVFAIL
TVOK:   LDI     H(AESTK)      ;SET P3 TO CURRENT
        XPAH    P3            ;STACK LOCATION
...
INSRT1: LD      @1(P3)        ; EACH CHAR IN LINE UP TO, BUT
        XRI     0x0D            ; NOT INCLUDING, CARR. RETURN
        JZ      INSRT2
        ILD     CHRNUM(P2)
        JMP     INSRT1
...
INSRT3: LD      CHRNUM(P2)    ;PUT NEW LINE LENGTH IN EX
        ...
        ANI     0x7F           ;NO - WE'LL INSERT LINE HERE,
...
TSTV     MACRO   FAIL
         DB      H((TSTVAR-1) & 0x1FFF)
         DB      L((TSTVAR-1) & 0x1FFF)
         DB      H(FAIL)
         DB      L(FAIL)
         ENDM
 
TSTN     MACRO   FAIL
         DB      H((TSTNUM-1) & 0x1FFF)
         DB      L((TSTNUM-1) & 0x1FFF)
         DB      H(FAIL)
         DB      L(FAIL)
         ENDM
...
PUTC2:  CSA                   ;SET STOP BIT
        ...
        HALT
        HALT
        JS      P3,REST       ;RETURN FROM ELBUG USING RU1FF0RU
