...
;*   BIT (BIT 7) 1S COMPLEMENTED. THIS ALLOWS DIRECT COMPARISON
...
;*   POINT REPRESENTATION ALLOWS DECIMAL NUMBERS IN THE APPROMIMATE
;*   RANGE OF 10**(-38) THROUGH 10**(38) WITH 6 TO 7 SIGNIFICANT
;*   DIGITS.
;*
;  LISTING REVISED: JUNE 14, 2023 ERICH KUESTER, KREFELD / GERMANY
;  the macro-assembler used here is a free tool by Alfred Arnold
;           http://john.ccac.rwth-aachen.de:8000/as/
;  assemble under Linux with $ ./build_6502.sh
;

; ALLOCATION OF RAM FOR VARIABLES

SIGN   = $3          ;BASE PAGE ADRESS
...
SAVET  LDA X1,X      ;SAVE EXP/MANTI AS T
       STA T,X
       LDA Z,X       ;LOAD EXP/MANTI WITH Z
       STA X1,X
...
TM2    LDA T,X
       ...
       DEX
       BPL TM2
       JSR FDIV      ;T*(Z-SORT(2)/(Z+SORT(2))
       LDX #3        ;4 BYTE TRANSFER
MIT    LDA X1,X
       STA T,X       ;COPY EXP/MANTI TO T AND
       ...
       LDX #3        ;4 BYTE TRANSFER
MIC    LDA C,X
       STA X1,X      ;LOAD EXP/MANTI WITH C
       DEX
       BPL MIC
       JSR FSUB      ;T*T-C
       LDX #3        ;4 BYTE TRANSFER
...
C      DCM 1.6567626
MHLF   DCM 0.5
PI     DCM 3.14159265 ;CIRCULAR NUMBER

;* EXP OF MANT/EXP1 RESULT IN MANT/EX ;P1
...
LB2    LDA B2,X
       ...
       DEX
       BPL LB2
       JSR FDIV      ;T=B2/(Z*Z+A2)
       LDX #3        ;4 BYTE MOVE
DLOAD  LDA X1,X
       STA T,X       ;SAVE EXP/MANTI AS T
       LDA C2,X
       STA X1,X      ;LOAD EXP/MANTI WITH C2
       LDA SEXP,X
       STA X2,X      ;LOAD EXP/MANT2 WITH SEXP
       DEX
       BPL DLOAD
       JSR FMUL      ;Z*Z*C2
       JSR SWAP      ;MOVE EXP/MANTI TO EXP/MANT2
       LDX #3        ;4 BYTE TRANSFER
LTMP   LDA T,X
       STA X1,X      ;LOAD EXP/MANTI WITH T
       DEX
       BPL LTMP
...
LD12   LDA MHLF,X
       ...
       RTS           ;RETURN ANS*(.5+Z/(-Z+D+C2*Z*Z-B2/(...)
...
ADD1   LDA M1,X
       ADC M2,X      ;ADD A BYTE OF MANT2 TO MANTI
       STA M1,X
...
;*
;* SUAP EXP/MANT1 WITH EXP/MANT2
;*
SWAP   LDX #$4      ;INDEX FOR 4-BYTE SUAP
SWAP1  STY E-1,X
...
;*
;*
;* CONVERT 16 BIT INTEGER IN M1(HIGH) AND Mi+1(LOW) TO
...
;*
FSUB   JSR FCOMPL    ;COMPL MANT1, CLEARS CARRY UNLESS ZERO
...
ADDEND BVC NORM      ;NO OVERFLOW,NOMALIZE RESULTS
...
;*
FMUL   JSR MD1       ;ABS. VAL OF THE MANT1, MANT2
...
DIV1   SEC           ;SET CARRY FOR SUBTRACT
       LDX #$2       ;INDEX FOR 3-BYTE INSTRUCTION
DIV2   LDA M2,X
       SBC E,X       ;SUBTRACT A BYTE OF E FROM MANT2
       PHA           ;SAVE ON STACK
       DEX           ;NEXT MORE SIGNIFANT BYTE
...
;*
       JSR RTAR      ;SHIFT MANT1 RIGHT AND INCREMENT EXPNT
