...
;
; 2023/08/08	ekuester
;		changes start address to $D000, supervisor is now at $D180,
;		 replaced spaces by tab stopps at positions 9, 17, 33 if possible
; 2023/10/16	ekuester
;		evaluates moving of messages, token table and I.L. table,
;		 reorganizes some subroutines on page D
;
; Last update: 2023/8/16, ekuester
...
SV_RESTRT	= (RESTRT -SPRVSR -1)	; RESTRT
SV_ERROUT	= (ERROUT -SPRVSR -1)	; ERROUT
SV_D157		= (L_D157 -SPRVSR -1)	; L_D157
SV_RTNEST	= (RTNEST -SPRVSR -1)	; RTNEST
SV_RTERRN	= (RTERRN -SPRVSR -1)	; RTERRN
...
ISCR:	XPAH	P1
	SCL
	CAI	0x10		; DECREMENT PAGE NO BY 1
	JNZ	AGAIN
	LDI	L(LETSGO)
	XPAL	P0		; SET PC LOW
...
RTERRN:	XAE			; RETURN WITH L(ERR#) IN AC
	LDI	0x1E
	JMP	RTN1		; to $D1AB
RTN:	ILD	-29(P2)
	ILD	-29(P2)
...
USING6:	LD	-1(P1)
	XRI	'"'		; TEST FOR '"'
	JZ	SV_RTRN(P3)
USING7:	LDI	L(M_SNTX)	; 'SNTX ERROR'
	JMP	SV_ERROUT(P3)
...
MESG	MACRO A,B
	...
	ORG	BASE+0x061C
MESGS	= $			; messages address for loading within code
...
;*	      I. L. TABLE	     *
;*************************************
	ORG	BASE+0x800	; LOW BYTE OF ADDRESS MUST BE 0x02
ILTABLE	= $
PROMPT: DO	GETLIN
...
NXT5:	LDI	0x26		; ONLY FOR ILTABLE LOW AT 0x00
	JMP	NEXT1
NEXT:	LDI	L(START+1)
NEXT1:	XAE
NEXT2:	LD	@1(P1)
	...
	LD	-1(P1)
	ADD	-1(P1)
	CAI	0x49		; ONLY FOR ILTABLE LOW AT 0x00
	JP	SYNT
	ADI	0x5D
	JMP	NEXT4
NEXT3:	LD	@-1(P1)
...
L_FF98: SCL
	...
	CAD	-21(P2)
	JP	L_FFAA
L_FF9F: LDI	'*'		; FILL WITH '*' IF FORMAT TOO SMALL
	CALL	P3,PUTASC
	DLD	-18(P2)
	JNZ	L_FF9F
	JMP	-42(P3)
